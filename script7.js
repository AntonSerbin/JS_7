...
function addInputTask1(number){
	...
	// function to the button plus and setting of limit of maximum inputs
	...
	document.getElementById("resultFieldTask1").innerHTML = "";
		...
		var inputTask1 = document.querySelectorAll("input.task1");
			...
			for (var i = 0; i < inputTask1.length; i++) {
					...
					if ((radioTask2[0].checked==true) && (i%2==0)) {document.getElementById("resultFieldTask1").innerHTML += `${inputTask1[i].value} `;};
					if ((radioTask2[1].checked==true) && (i%2!=0)) {document.getElementById("resultFieldTask1").innerHTML += `${inputTask1[i].value} `;};
					if (radioTask2[2].checked==true) {document.getElementById("resultFieldTask1").innerHTML += `${inputTask1[i].value} `;};
								};
							};
// end tasks 1-3
// task 4 start
...
// task 5 end; task 6 start								
	...
	function displayBlock(){divText6.classList.add('displayBlock');};
	function hideBlock(){setTimeout(function(){divText6.classList.remove('displayBlock')},2500);};
	divTask6.addEventListener("mouseleave", function(){hideBlock();});
	divTask6.addEventListener("mouseenter", function(){displayBlock()});
// task 6 end; task 7,8 start	
	...
	function infoImage(image){
			...
			if (image.alt.length==0 ){			image.style.border = "solid 3px red";
												image.style.opacity = "0.75";}
			};

	var links7 = document.querySelectorAll("a");
		for (var i = 0; i < links7.length; i++) {
			links7[i].addEventListener("mouseenter", function(){infoLink(this);});
		};
...
var quantityHTask9 = document.querySelectorAll("h1,h2,h3,h4,h5,h6");
	if (quantityHTask9.length!=0) {resultFieldTask9.innerHTML = `Quantity of H : ${quantityHTask9.length} pcs`}
